...
- 관리자 운영 데이터는 프로젝트 단위로 이동하는 번들입니다. 프로젝트 관리와 어드민
  계정처럼 workspace-level인 API를 제외한 관리자 운영 API는
  `/api/admin/projects/{projectId}/...` 하위 구조를 기본으로 합니다.
  프로젝트 하위 구조가 아닌 운영 API가 `projectId`를 누락하면
  `PROJECT_ID_REQUIRED` 오류를 반환합니다.
- `projectId` 위치 규칙:
  - 프로젝트 자체 리소스는 `/api/admin/projects/{projectId}` path parameter를 사용합니다.
  - 관리자 운영 데이터는 `/api/admin/projects/{projectId}/{resource}` 하위 path를 사용합니다.
  - 근로자 로그인/온보딩처럼 인증 요청 payload로 프로젝트를 선택하는 worker-facing API는
    request body에 `projectId`를 둡니다.
  - 검색, 날짜, 상태 같은 필터만 query parameter로 둡니다.
...
`projectId` scopes the category setting to one project. `category` is the
employment-type value recorded in that project's admin worker ledger.
When an admin creates a worker registration or imports XLSX rows, a new category value
is automatically added to the selected project's category settings if it does not
already exist. `company` and `team` are normalized free-text worker attributes for
the first pass, not separately managed catalogs. Worker signup category choices use
the selected project's current category settings. Document submission display is computed from
...
`startDate`와 `endDate`는 프로젝트 기간입니다. `eventStartDate`와
`eventEndDate`는 프로젝트 안에서 실제 행사가 열리는 행사 기간입니다.
...
응답이 빈 배열이면 관리자 프론트엔드는 프로젝트 관리 탭으로 이동해 첫 프로젝트
생성을 바로 안내합니다.
...
없으면 `null`을 반환합니다. 이 pure 상태에서는 관리자 운영 데이터 API를 호출하지
않고 프로젝트 생성 flow를 먼저 완료합니다.
...
`GET /api/projects/{projectId}/worker-categories`

Lists category settings that workers may choose during signup or first login. Categories
are project-scoped and are created automatically from admin worker registrations and
XLSX imports for the selected project.

Response `200 OK`: `WorkerCategorySetting[]`

### List Admin Worker Categories

`GET /api/admin/projects/{projectId}/worker-categories`

Lists all category settings for admin management in one project. Registration/signup
allow toggles are not managed by operators; every recorded employment type remains
usable. Company and team are not managed as nested category catalogs in the first pass.

Response `200 OK`: `WorkerCategorySetting[]`

### List Admin Schedule Columns

`GET /api/admin/projects/{projectId}/schedule-columns`
...
`POST /api/admin/projects/{projectId}/schedule-columns`
...
`DELETE /api/admin/projects/{projectId}/schedule-columns/{id}`
...
`POST /api/admin/projects/{projectId}/worker-categories`
...
`POST /api/admin/projects/{projectId}/worker-categories/rename`
...
`DELETE /api/admin/projects/{projectId}/worker-categories/{category}`

Deletes a project category only when no worker registration in that project uses it.
...
`GET /api/admin/projects/{projectId}/worker-registrations`

Admin worker ledger with onboarding, document, and QR usage state for one project.
Workspace-wide aggregation must use a separate explicit admin reporting API rather
than omitting `projectId`.

Response `200 OK`: `WorkerRegistration[]`

### Create Worker Registration

`POST /api/admin/projects/{projectId}/worker-registrations`
...
`PATCH /api/admin/projects/{projectId}/worker-registrations/{uid}`
...
The path `projectId` is the selected admin project context. Cross-project worker moves are not
handled by this endpoint, and request bodies must not move the worker to a different project.
...
`DELETE /api/admin/projects/{projectId}/worker-registrations/{uid}`
...
`POST /api/admin/projects/{projectId}/worker-registrations/import-xlsx`
...
settings for the selected project. B(No.) and G(resident registration number) are ignored
...
`GET /api/admin/projects/{projectId}/qr-usage/summary?date=2026-05-26&mealType=all`
...
`GET /api/admin/projects/{projectId}/qr-usage/logs?date=2026-05-26&workerId={workerUid}&qrType=meal&result=allowed`

운영 예정 API입니다. 관리자 식권/생수 QR 사용 현황 화면의 전체 스캔 로그를
반환합니다. `date`, `workerId`, `qrType`, `result`로 필터링할 수 있고,
`workerId`를 생략하면 프로젝트 전체 로그를 반환합니다.

Response `200 OK`: `QrUsageEvent[]`

### Get Worker QR Usage Detail

`GET /api/admin/projects/{projectId}/workers/{workerId}/qr-usage?date=2026-05-26`

운영 예정 API입니다. 관리자 화면에서 사용자를 검색했을 때 해당 사용자의 식권/생수
사용량과 스캔 로그를 함께 반환합니다. `workerId`는 전역 UID이지만 관리자 운영
조회는 항상 선택 프로젝트 컨텍스트와 함께 실행합니다.
...
- `qr_tokens`: 프로젝트/근로자별 정적 QR 토큰과 활성 상태
- `qr_entitlements`: 프로젝트/근로자/날짜/QR 유형별 지급권과 사용량
- `qr_usage_events`: 프로젝트와 연결된 모든 스캔 성공/거부 로그

### Get Admin Weather Overview

`GET /api/admin/projects/{projectId}/weather`

관리자 기상 정보 관리 탭에서 현재 기상, 24시간 예보, 알림 로그, 관측 지점,
자동 경보 임계값을 함께 조회합니다. 현재 구현은 실제 기상청 호출 전 단계의
백엔드 어댑터 placeholder이며, 프론트엔드는 이 API만 호출합니다. 프로젝트별로
...
`POST /api/admin/projects/{projectId}/weather/station`
...
`POST /api/admin/projects/{projectId}/weather/thresholds`
...
`GET /api/admin/projects/{projectId}/payroll-documents`

관리자 근로자 관리 화면에서 서류 제출 필수 대상자와 제출 상태를 확인하기 위한
프로젝트별 목록입니다. 응답에는 원본 파일 경로나 장기 다운로드 토큰을 포함하지 않습니다.
...
`POST /api/admin/projects/{projectId}/payroll-documents/{workerId}/files/{fileId}/open`

관리자가 특정 근로자의 제출 서류를 열람하기 위한 임시 접근 정보를 발급합니다.
선택 프로젝트는 path의 `projectId`로 전달합니다.

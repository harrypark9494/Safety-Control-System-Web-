...
- 관리자 운영 데이터는 프로젝트 단위로 이동하는 번들입니다. 프로젝트 관리와 어드민
  계정처럼 workspace-level인 API를 제외한 관리자 운영 API는 `projectId`를 필수
  컨텍스트로 받으며, 누락 시 `PROJECT_ID_REQUIRED` 오류를 반환합니다.
...
`GET /api/worker-categories?projectId=waterbomb-2026-summer`
...
`GET /api/admin/worker-categories?projectId=waterbomb-2026-summer`
...
`DELETE /api/admin/worker-categories/{category}?projectId=waterbomb-2026-summer`
...
`POST /api/admin/worker-registrations/import-xlsx?projectId=waterbomb-2026-summer`
...
`GET /api/admin/workers/{workerId}/qr-usage?projectId=waterbomb-2026-summer&date=2026-05-26`
...
`GET /api/admin/payroll-documents?projectId=waterbomb-2026-summer`
...
Request body 또는 query에는 선택 프로젝트의 `projectId`를 포함합니다.

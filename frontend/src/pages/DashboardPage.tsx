    ...
    <section className="tab-panel">
      ...
      <section className="app-card emergency-card">
        ...
        <div className="manager-call">
          ...
          <div><strong>박재형 매니저</strong><p>010-2344-9089</p></div>
          <a href="tel:01023449089" aria-label="박재형 매니저에게 전화"><Icon name="phone" /></a>
        </div>
...
function ProfileTab({ worker, logout }: { worker: ReturnType<typeof getSession>; logout: () => void }) {
  const name = worker?.role === "worker" ? worker.name : "박재형";
  const phone = worker?.role === "worker" ? worker.phone : "010-2344-9089";

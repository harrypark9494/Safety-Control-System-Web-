...
- `GET /api/health`
- `GET /api/worker-categories?projectId=...`
- `GET /api/admin/worker-categories?projectId=...`
- `GET /api/admin/schedule-columns?projectId=...`
- `POST /api/admin/worker-categories`
- `POST /api/admin/worker-categories/rename`
- `DELETE /api/admin/worker-categories/:category?projectId=...`
- `GET /api/admin/worker-registrations?projectId=...`
...
프로젝트 관리와 어드민 계정을 제외한 운영 데이터 API는 선택 프로젝트의 `projectId`를
필수 컨텍스트로 받습니다.
로컬에서 완전 빈 pure 상태를 확인할 때는 `ENABLE_DEMO_SEED_DATA=false`로 데모
프로젝트/근로자/스케줄 시드를 끌 수 있습니다.
...
- `qr_tokens`: 프로젝트/근로자별 정적 QR 토큰, QR 유형, 활성 상태
- `qr_entitlements`: 프로젝트/근로자/날짜/QR 유형별 지급권, 사용량, 잔여 상태
- `qr_usage_events`: 프로젝트와 연결된 전체 스캔 로그. 성공과 거부 이력을 모두 저장

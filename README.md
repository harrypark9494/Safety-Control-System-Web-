...
| --- | --- | --- |
| Log in | `src/pages/LoginPage.tsx` | `/`, `/login/` |
| Dashboard | `src/pages/DashboardPage.tsx` | `/dashboard/` |
| Admin Desktop | `src/pages/AdminPage.tsx` | `/admin/` |
| Payroll Documents | `src/pages/PayrollDocumentsPage.tsx` | `/payroll-documents/?demo=1` |
...
Firebase Hosting 배포 대상은 Vite 빌드 결과인 `dist/`입니다. `src/`는 실제
React/TypeScript 앱 소스이고, `demos/`는 기존 mock 데모 보존용으로 둡니다.

Dashboard의 현재 React 데이터는 `src/data/demoData.ts`에 mock 데이터로
...
로그인 화면은 `src/features/auth/session.ts`의 mock 세션 어댑터를 사용합니다.
